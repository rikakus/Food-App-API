...
const usersController = {
  ...
  detail: (req, res) => {
    try {
      ...
      console.log(req.APP_DATA.tokenDecode.id);
      if (isNaN(check)) {
  ...
  update: async (req, res) => {
    try {
      ...
      usersModel
        ...
        .then((result) => {
          success(res, result.command, "success", "success to update");
        })
        .catch((err) => {
  ...
  userControl: async (req, res) => {
    try {
      ...
      if (isActive != "1" && isActive != "0") {
        throw Error("is_active must be 0 or 1");
      }

      isActive = parseInt(isActive);
      usersModel
        .userControl(id, isActive)
        .then((result) => {
          success(res, result.command, "success", "success to deactive");
        })
        .catch((err) => {
          failed(res, err.message, "failed", "failed to deactive");
        });
    } catch (err) {
      failed(res, err.message, "failed", "failed to deactive");
    }
